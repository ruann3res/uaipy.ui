...
export class SensorService {
  static async createSensor(sensor: Omit<SensorType, "id">) {
    const { data: response } = await httpClient.post<SensorType>(
      "/sensors",
      sensor
    );
    return response;
  ...
  static async updateSensor(id: string, sensor: Partial<SensorType>) {
    const { data: response } = await httpClient.put<SensorType>(
      `/sensors/${id}`,
      sensor
    );
    return response;
  ...
  static async getById(id: string) {
    const { data: response } = await httpClient.get<SensorType>(
      `/sensors/${id}`
    );
    return response;
  }

  static async deleteSensor(id: string) {
